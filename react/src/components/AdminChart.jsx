...
export default function AdminChart({ apiUrl }) {
  ...
  if (data.length === 0) return <p className="dw-empty">Belum ada data transaksi dari pengguna.</p>;
  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
        ...
        <Legend formatter={(v) => (v === 'masuk' ? 'Pemasukan' : 'Pengeluaran')} wrapperStyle={{ fontSize: 12 }} />
        <Bar dataKey="masuk" fill="#2D6A4F" radius={[4, 4, 0, 0]} />
        <Bar dataKey="keluar" fill="#C1121F" radius={[4, 4, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  );
}
